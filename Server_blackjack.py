...
class Blackjack:
    ...
    def run(self):
        ...
        for idx, player in enumerate(self.active_players):
            ...
            self.send_data(player, f"{OUTPUT_HEADER}You now have ${player.player_money}!")
    ...
    def run_round(self):
        ...
        for player in self.active_players:
            self.send_data(player, f"{OUTPUT_HEADER}The dealer now flips over their hole card.")
        ...
        if self.deck.sum_cards(self.dealer_cards) < 17:
            while self.deck.sum_cards(self.dealer_cards) < 17:
                self.dealer_cards.append(self.shuffled_deck.pop(0))
            for player in self.active_players:
                self.send_data(player, f"{OUTPUT_HEADER}The dealer has less than 17 points. he begins drawing.")
                self.print_dealer_info(player)

        for player in self.active_players: self.send_data(player, f"{OUTPUT_HEADER}Game over!")
        return self.game_end(choices)
        # else:
        #     # If one person has a blackjack, the game immediately ends
        #     self.send_data(f"{OUTPUT_HEADER}Game immediately over!")
    ...
    def thread1(self, idx, player, bets):
        self.send_data(player, f"{OUTPUT_HEADER}You have ${player.player_money} to bet with!")
        self.send_data(player,
                       f"{QUESTION_HEADER_INPUT} How much would you like to bet?,"
                       f"{player.player_money},{self.min_bet}")
        bets[idx] = int(self.receive_data(player))
        self.send_data(player,
    ...
    def thread3(self, player):
        ...
        if player.player_money >= self.min_bet:
            content = QUESTION_HEADER_CHOICE + ",".join(choices)
            self.send_data(player, content)
            choice = self.receive_data(player)
        ...
        if choice == choices[2]:
            ...
            self.send_data(player, DISCONNECT_MESSAGE)
            player.conn.close()
    ...
    def game_end(self, choices):
        ...
        for idx, player in enumerate(self.active_players):
            ...
            if special == 4:
                ...
                self.send_data(player, f"{OUTPUT_HEADER}The player chose to surrender! They get half their bet back!")
                multipliers[idx] = -0.5
            elif players_sum[idx] > 21:
                self.send_data(player, f"{OUTPUT_HEADER}The player went bust! The player lost!")
                multipliers[idx] = -1
            elif dealer_sum > 21:
                self.send_data(player, f"{OUTPUT_HEADER}The dealer went bust! The player won!")
                multipliers[idx] = 1
                ...
                self.send_data(player, f"{OUTPUT_HEADER}The player has more points than the dealer! The player won!")
                multipliers[idx] = 1
            elif players_sum[idx] < dealer_sum:
                self.send_data(player, f"{OUTPUT_HEADER}The player has less points than the dealer! The dealer won!")
                multipliers[idx] = -1
    ...
    def blackjack_check(self):
        ...
        for idx, player in enumerate(self.active_players):
            ...
            if player_sum[idx] == 21 and dealer_sum == 21:
                self.send_data(player, f"{OUTPUT_HEADER}The player and dealer both have a blackjack! A push occurred!")
                player.status = "Push"
            if dealer_sum == 21:
                self.send_data(player,
            ...
            if player_sum[idx] == 21:
                self.send_data(player,
                               f"{OUTPUT_HEADER}The player has a blackjack and the dealer does not! The player gets payed out 3 to 2!")
    ...
    def broadcast(self, message_type, list=[]):
        ...
        for player in self.connected_players:
            self.send_data(player, broadcast_messsage)
